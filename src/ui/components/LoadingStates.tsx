...
export const ShimmerLoading = ({ height = '200px', width = '100%' }: { height?: string; width?: string }) => (
  <Box
    height={height}
    width={width}
...
export const DotsLoading = ({ size = 'md', color = 'blue.500' }: { size?: 'sm' | 'md' | 'lg'; color?: string }) => {
  const dotSize = {
    sm: '6px',
    md: '8px',
    lg: '12px'
...
export const FormLoadingOverlay = ({ isLoading, children }: { isLoading: boolean; children: React.ReactNode }) => (
...
export const ListLoading = ({ count = 5, itemHeight = '60px' }: { count?: number; itemHeight?: string }) => (
  <Box>
    {Array.from({ length: count }).map((_, index) => (
      <Flex
        key={index}
